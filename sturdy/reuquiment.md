hiện tại sẽ có những yêu cầu sau:
1. chỗ lớp 2 sẽ thêm một button [thêm cấu trúc]
Khi mà thêm cấu trúc sẽ cho họ chọn loại vật liệu, đảm bảo là 2 loại vật liệu phải chung độ dày, nên là khi tính độ dày phải xem xét cả 2 á
2. Khi mà hiển thị kết quả đặc tả nguyên vật liệu, ở phần nào có nhiều cấu trúc sẽ ghi ra cấu trúc đó luôn
Ví dụ 1 cấu trúc:
lớp 2: pet (nội dung)
       mpet (nội dung)
Ví dụ 2 cấu trúc: Tại vì 2 con hình người ta sẽ xắp xếp liền mặt với nhau
    Pet (nd)
    Mpet (nd)
    Pet(nd)
    Lưu ý: khi tính sẽ tính riêng hết vì giá mỗi vật liệu vớ khổ khác nahu sẽ khác nhau, và phi hao chúng sẽ giống nhau đó nha
Chi phí của lớp 2 = tổng chi phí từng vật liệu
Lưu ý t còn có cái đọa dư ra 2 bên đó
