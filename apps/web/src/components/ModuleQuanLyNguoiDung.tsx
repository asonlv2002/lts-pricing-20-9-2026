...
export default function UserManagementModule() {
  ...
  // Tải/lưu users từ localStorage
  const LS_USERS = 'lts_users';
  const loadUsers = () => {
    try { const raw = window.localStorage.getItem(LS_USERS); return raw ? JSON.parse(raw) as SafeUser[] : []; }
    catch { return []; }
  };
  const saveUsers = (list: SafeUser[]) => {
    try { window.localStorage.setItem(LS_USERS, JSON.stringify(list)); } catch {}
  };
  const fetchUsers = () => setUsers(loadUsers());
  ...
  const handleAdd = async (data: Partial<SafeUser> & { password?: string }) => {
    ...
    try {
      ...
      saveUsers([mucMoi, ...loadUsers()]);
      fetchUsers(); closeModal(); showToast('✅ Tài khoản đã được tạo.');
  ...
  const handleEdit = async (data: Partial<SafeUser> & { password?: string }) => {
    ...
    try {
      ...
      fetchUsers(); closeModal(); showToast('✅ Đã cập nhật tài khoản.');
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      saveUsers(loadUsers().filter(u => u.id !== selected.id));
      fetchUsers(); closeModal(); showToast('✅ Đã xóa tài khoản.');
