...
const CW = { stt: '4.5%', name: '18%', desc: '38%', unit: '6%', qty: '9.5%', price: '12.5%', total: '11.5%' };
const BLUE = '#3d85c6';
const BORDER = '1px solid #333';

const styles = StyleSheet.create({
  page: {
    fontFamily: FONT, fontSize: 10, paddingTop: 56, paddingBottom: 42, paddingLeft: 42, paddingRight: 42,
    lineHeight: 1.15,
  },
  // ── Company header ──
  coName: { textAlign: 'center', fontSize: 12, fontWeight: 700, marginBottom: 2 },
  coAddr: { textAlign: 'center', fontSize: 10, marginBottom: 2 },
  coTax: { textAlign: 'center', fontSize: 10, marginBottom: 12 },
  // ── Title ──
  title: { textAlign: 'center', fontSize: 16, fontWeight: 700, marginTop: 6, marginBottom: 2 },
  ...
  // ── Table ──
  table: { width: '100%', marginVertical: 8, borderLeft: BORDER, borderTop: BORDER },
  tableRow: { flexDirection: 'row' },
  th: {
    backgroundColor: BLUE, color: '#fff', fontWeight: 700, textAlign: 'center',
    padding: '3 4', borderRight: BORDER, borderBottom: BORDER, fontSize: 10,
  },
  td: {
    padding: '3 4', borderRight: BORDER, borderBottom: BORDER, fontSize: 10,
  },
  ...
  totalLabel: { width: '88.5%', textAlign: 'right', padding: '3 4', borderRight: BORDER, borderBottom: BORDER, fontSize: 10 },
  totalLabelBold: { width: '88.5%', textAlign: 'right', padding: '3 4', borderRight: BORDER, borderBottom: BORDER, fontWeight: 700, fontSize: 10 },
  totalValue: { width: '11.5%', textAlign: 'right', padding: '3 4', borderRight: BORDER, borderBottom: BORDER, fontSize: 10 },
  totalValueBold: { width: '11.5%', textAlign: 'right', padding: '3 4', borderRight: BORDER, borderBottom: BORDER, fontWeight: 700, fontSize: 10 },
  // ── Xem tiếp ──
  xemTiep: { textAlign: 'center', fontStyle: 'italic', color: '#888', padding: '3 4', borderRight: BORDER, borderBottom: BORDER, fontSize: 10 },
  // ── Notes ──
  luuY: { fontWeight: 700, fontSize: 11, marginTop: 14, marginBottom: 4 },
  luuYItem: { fontSize: 10, marginVertical: 1 },
  // ── Signatures ──
  sigRow: { textAlign: 'center', fontSize: 11, marginTop: 30 },
  sigSub: { textAlign: 'center', fontSize: 10, marginVertical: 2 },
  // ── Number in words ──
  soChu: { fontSize: 10, padding: '3 4', borderRight: BORDER, borderBottom: BORDER },
});
...
function soSangChu(n: number): string {
  ...
  function docNhom(so: number): string {
    ...
    if (chuc === 0) { if (tram > 0 && donvi > 0) parts.push('linh'); }
...
interface HistoryItem {
  ...
  chotGia?: number;
  quoteCode?: string;
  terms?: { vatRate: number; vatCustom?: number; vatCylinderRate?: number; paymentTerms?: string; deliveryTime?: string; notes?: string };
...
function buildBagSpecDescription(spec: any, input: any, structure: string, totalThickness = 0): string {
...
function HeaderRow() {
  return (
    <View style={styles.tableRow}>
      <View style={[styles.th, { width: CW.stt }]}><Text>STT</Text></View>
      <View style={[styles.th, { width: CW.name }]}><Text>Tên hàng</Text></View>
      <View style={[styles.th, { width: CW.desc }]}><Text>Mô tả</Text></View>
      <View style={[styles.th, { width: CW.unit }]}><Text>ĐVT</Text></View>
      <View style={[styles.th, { width: CW.qty }]}><Text>Số lượng</Text></View>
      <View style={[styles.th, { width: CW.price }]}><Text>Đơn giá VNĐ</Text></View>
      <View style={[styles.th, { width: CW.total }]}><Text>Thành tiền VNĐ</Text></View>
    </View>
  );
}

function ProductRow({ stt, group, tier, idx, totalRows }: {
  stt: number; group: ProductGroup; tier?: { quantity: number; unitPrice: number; total: number };
  idx: number; totalRows: number;
}) {
  const isFirst = idx === 0;
  ...
  if (isFirst) {
    return (
      <View style={styles.tableRow} key={idx}>
        <View style={[styles.td, styles.tdAC, { width: CW.stt }]}><Text>{stt}</Text></View>
        <View style={[styles.td, { width: CW.name }]}><Text>{group.productName}</Text></View>
        <View style={[styles.td, styles.tdPL, { width: CW.desc }]}><Text>{group.description}</Text></View>
        <View style={[styles.td, styles.tdAC, { width: CW.unit }]}><Text>{group.isBag ? 'Túi' : 'm²'}</Text></View>
        <View style={[styles.td, styles.tdAR, { width: CW.qty }]}><Text>{dinhDangSo(tier!.quantity)}</Text></View>
        <View style={[styles.td, styles.tdAR, { width: CW.price }]}><Text>{dinhDangSo(tier!.unitPrice)}</Text></View>
        <View style={[styles.td, styles.tdAR, { width: CW.total }]}><Text>{dinhDangSo(tier!.total)}</Text></View>
      </View>
    );
  }
  if (isTier) {
  ...
  if (isCyl && cyl) {
    return (
      <View style={styles.tableRow} key={idx}>
        <View style={[styles.td, { width: CW.stt }]} />
        <View style={[styles.td, { width: CW.name }]}><Text>{cyl.name}</Text></View>
        <View style={[styles.td, { width: CW.desc }]}><Text>{cyl.dims}</Text></View>
        <View style={[styles.td, styles.tdAC, { width: CW.unit }]}><Text>trục</Text></View>
        <View style={[styles.td, styles.tdAR, { width: CW.qty }]}><Text>{dinhDangSo(cyl.qty)}</Text></View>
        <View style={[styles.td, styles.tdAR, { width: CW.price }]}><Text>{dinhDangSo(cyl.unitPrice)}</Text></View>
        <View style={[styles.td, styles.tdAR, { width: CW.total }]}><Text>{dinhDangSo(cyl.total)}</Text></View>
      </View>
    );
  }
  return null;
...
function TotalRow({ label, value, bold }: { label: string; value: number; bold?: boolean }) {
  return (
    <View style={styles.totalRow}>
      <View style={bold ? styles.totalLabelBold : styles.totalLabel}><Text>{label}</Text></View>
      <View style={bold ? styles.totalValueBold : styles.totalValue}><Text>{dinhDangSo(Math.round(value))}</Text></View>
    </View>
  );
}
...
function BaoGiaPage({ item, page, totalPages, pageGroups, customerInfo }: {
  item: HistoryItem;
  page: number;
  totalPages: number;
  ...
  for (const g of pageGroups) {
    ...
    for (let i = 0; i < groupRows; i++) {
      ...
      tableRows.push(
        <ProductRow key={`${stt}-${i}`} stt={stt} group={g} tier={tier} idx={i} totalRows={groupRows} />
      );
    }
  }
  ...
  return (
    <Page size="A4" style={styles.page}>
      {/* Company header */}
      <Text style={styles.coName}>CÔNG TY CỔ PHẦN THƯƠNG MẠI VÀ SẢN XUẤT BAO BÌ LAI TRƯỜNG SƠN- LONG AN</Text>
      <Text style={styles.coAddr}>SỐ 36, ĐƯỜNG ẤP 7B, XÃ MỸ YÊN, TỈNH TÂY NINH, VIỆT NAM</Text>
      <Text style={styles.coTax}>MST: 1101904518                    Mail: baobilaitruongson.la@gmail.com</Text>
      ...
      <View style={styles.table}>
        ...
        {isLast ? (
          <>
            {(() => {
              const allTierTotal = allGroups.reduce((s, g) => s + g.tiers.reduce((ss, t) => ss + t.total, 0), 0);
              const allCylTotal = allGroups.reduce((s, g) => s + (g.cylinder?.total || 0), 0);
                ...
                <>
                  ...
                  <View style={styles.totalRow}>
                    <View style={[styles.soChu, { width: '100%' }]}>
                      <Text>Số tiền (viết bằng chữ): {soSangChu(Math.round(tongCong))}</Text>
                    </View>
                  </View>
                </>
              );
        ...
        <>
          {item.terms ? (
            <>
              ...
              <Text style={styles.luuYItem}>- Số lượng thành phẩm có thể tăng hoặc giảm so với đơn đặt hàng: ±10%</Text>
              {vat > 0 || vatTruc > 0 ? (
                <Text style={styles.luuYItem}>- Thuế VAT: {vat}% đối với hàng hóa, {vatTruc}% đối với trục in</Text>
              ) : (
                <Text style={styles.luuYItem}>- Chưa bao gồm thuế VAT</Text>
              )}
              {item.terms.paymentTerms ? <Text style={styles.luuYItem}>- Thanh toán: {item.terms.paymentTerms}</Text> : null}
              {item.terms.deliveryTime ? <Text style={styles.luuYItem}>- Thời gian giao hàng: {item.terms.deliveryTime}</Text> : null}
              {item.terms.notes ? <Text style={styles.luuYItem}>- Ghi chú: {item.terms.notes}</Text> : null}
            </>
          ) : null}
          <Text style={styles.sigRow}>NGƯỜI LẬP                              PHÊ DUYỆT                              KHÁCH HÀNG</Text>
          <Text style={styles.sigSub}>(ký, họ tên)                               (ký, họ tên)                               (ký, họ tên)</Text>
        </>
      )}
    </Page>
  );
...
export function BaoGiaPdfDocument({ item, customerInfo }: BaoGiaPdfDocumentProps) {
  const products: QuoteProductLine[] = item.quoteProducts?.length
    ? item.quoteProducts
    : [{
        sourceHistoryItemId: item.id, productName: item.productName, structure: item.structure,
...
export function BaoGiaReactPdfDownload({ item, customerInfo, className }: BaoGiaReactPdfDownloadProps) {
  return (
    <PDFDownloadLink
