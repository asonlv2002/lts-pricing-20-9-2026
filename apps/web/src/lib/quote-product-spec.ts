...
export type BagSealKind = 'head' | 'bottom';
...
export type BagSpecConditionalField = 'gusset' | 'backSeal' | 'standupBottom' | 'sideSeal';
...
export function shouldShowBagSpecField(bagType: string, field: BagSpecConditionalField): boolean {
  ...
  if (field === 'sideSeal') return bagType !== 'cutSeal';
