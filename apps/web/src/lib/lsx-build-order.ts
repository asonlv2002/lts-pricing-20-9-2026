...
export interface BuildLsxOrderCtx {
  ...
  profitTable: ProfitRow[];
  smallWidthPrices: SmallWidthMaterialPrice[];
  /** ProductionOrder[] | LsxRow[] | string[] — dùng gen MSP (TP_######). */
...
export function buildManualFromSource(
  ...
  const i = source.input;
  // Số LSX do server cấp (versionByMonth) — FE fill sau khi tạo order
  // (soLsxTuOrder), không gen client-side nữa.
  const m = defaultManual('', ctx.preparedBy);
